...
def apply_put_series(data, grouper, func, out=None, *args, **kwargs):

    # grab first result to find dtype
    first = grouper.bins[0]
    res = func(data[0:first])
    if out is None:
    ...
    for bin in grouper.bins[1:]:
        out[start:bin] = func(data[start:bin])
        start = bin
...
def apply_put_frame(data, grouper, func, *args, **kwargs):
    ...
    for col in data.columns:
        if col not in func:
            continue
        f = func[col]
        s = apply_put_series(data[col], grouper, f)
        sdict[col] = s
